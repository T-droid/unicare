...
import { db } from "../db";
import { UserTable, PatientMedicalRecords, labTestRequestTable } from "../db/schema";
...
export async function getStudentMedicalHistory(regNo: string) {
    try {
      ...
      const medicalHistory = await db
        ...
        .where(eq(labTestRequestTable.medical_history_id, PatientMedicalRecords.id))
...
export async function createStudentPrescription(regNo: string, prescription: string) {
    try {
        return await db
...
export async function requestStudentLabTest(
    ...
    try {
      ...
      const medicalHistoryId = medicalHistory.length > 0 ? medicalHistory[0].id : null;
...
export async function updatePatientType(patientType: "outpatient" | "inpatient", regNo: string) {
    try {
