...
// role ENUM
export const userRoleEnum = pgEnum("user_role", ["doctor", "nurse", "receptionist", "lab_technician"]);
...
export const DepartmentsTableRelations = relations(DepartmentsTable, ({ many }) => ({
...
export const AppointmentsTable = pgTable("appointments", {
    id: uuid("id").defaultRandom().primaryKey(),
    student_id: uuid("student_id").references(() => StudentTable.id, { onDelete: "cascade" }).notNull(),
    doctor_id: uuid("doctor_id").references(() => UserTable.id, { onDelete: "cascade" }).notNull(),
